...
async function seedAdminUser() {
    if (!adminPassword) {
        throw new Error('Missing SEED_ADMIN_PASSWORD. Add it to .env before running the seed.');
    }
...
async function seedActiveScoringCycle() {
    ...
    const activeCycle = scoringCycle
        ? await prisma.scoringCycle.update({
            ...
            data: {
                status: client_1.ScoringCycleStatus.ACTIVE,
    ...
    await prisma.scoringCycle.updateMany({
        ...
        data: { status: client_1.ScoringCycleStatus.DRAFT },
    });
}
async function main() {
    await seedAdminUser();
